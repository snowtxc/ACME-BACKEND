...
namespace BusinessLayer.BLs
{
    public class BL_Compra : IBL_Compra
    {
        ...
        public async Task<EstadoCompraDto> pasarAlSiguienteEstado(int compraId)
        {
            ...
            if (lastEstado.EstadoCompraId == 3 && compra.MetodoEnvio == MetodoEnvio.DireccionPropia)
            {
                nuevoEstado = await _estadoCompra.getById(lastEstado.EstadoCompraId + 2);
            }
            else
            {
                nuevoEstado = await _estadoCompra.getById(lastEstado.EstadoCompraId + 1);
            }
            Usuario cliente = compra.Usuario;
